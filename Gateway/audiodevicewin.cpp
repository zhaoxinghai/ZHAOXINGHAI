...
bool CAudioDevice::InitDevice(bool bCapture,int nChannel,int nBitsPerSamp)
{
    ...
    if (FAILED(hr))
    {
        printf("%s", "m_pAudioClient->SetEventHandle");
    ...
    if(m_bCapture)
    {
        ...
        if (FAILED(hr))
        {
            printf("%s", "m_pAudioClient->GetService");
            m_error = ERROR_AUDIO_DEVICE;
            return false;
        }
    }
...
bool CAudioDevice::StartPlayBack(CThreadDevice * pMySource)
{
    ...
    if (FAILED(hr))
    {
        printf("%s", "m_pRenderClient->GetBuffer");
        m_error = ERROR_AUDIO_DEVICE;
        return false;
    }
    ...
    if (!bLoadData)
    {
        printf("%s", "pMySource->LoadData first");
        m_error = ERROR_AUDIO_DEVICE;
        return false;
    }
    ...
    if (FAILED(hr))
    {
        printf("%s", "m_pRenderClient->ReleaseBuffer");
        m_error = ERROR_AUDIO_DEVICE;
        return false;
    }
...
bool CAudioDevice::InitAudioClient()
{
    ...
    if (FAILED(hr))
    {
        printf("%s", "CLSID_MMDeviceEnumerator-CoCreateInstance");
        return false;
    }
    ...
    if (FAILED(hr))
    {
        printf("%s", "m_pEnumerator->GetDefaultAudioEndpoint");
        return false;
    }
    ...
    if (FAILED(hr))
    {
        printf("%s", "m_pAudioClient->GetMixFormat");
        return false;
    }
...
bool CAudioDevice::InitPeriod()
{
    ...
    if(FAILED(hr))
    {
        printf("%s", "m_pAudioClient->IsFormatSupported");
        return false;
    }
    ...
    if (FAILED(hr))
    {
        printf("%s", "m_pAudioClient->GetDevicePeriod");
        return false;
    }
   ...
   switch (hr)
    ...
    default:
        printf("%s", "m_pAudioClient->Initialize");
        return false;
    }
    return true;
}
