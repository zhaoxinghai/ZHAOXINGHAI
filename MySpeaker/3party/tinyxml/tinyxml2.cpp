...
const char* XMLDocument::GetErrorDetail() const
{
    if ( Error() ) {
        static const int MAX_ERR_LEN = 1536;
        static const int LEN = 512;
        static char err[MAX_ERR_LEN] = {0};
        char buf1[LEN] = { 0 };
        char buf2[LEN] = { 0 };
        ...
        TIXML_SNPRINTF( err, MAX_ERR_LEN, "XMLDocument error id=%d '%s' line=%d str1=%s str2=%s",
            static_cast<int>( _errorID ), ErrorName(), _errorLineNum, buf1, buf2 );
