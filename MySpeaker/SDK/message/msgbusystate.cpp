...
CMsgBusystate::CMsgBusystate(CTSystem* pSys):CMsgBase(pSys)
{
    m_Mid = PAMSG_MID_BUSY_STATE;
    m_SubMid = 0;
}

CMsgBusystate::~CMsgBusystate()
{

}

void CMsgBusystate::OnRespond(unsigned char* pBuf, const int len)
{
    /*
    m_SubMid = pBuf[0];
    if (m_Mid == 0x01)
...
void CMsgBusystate::OnLocalSourceUsageReport11(unsigned char* pBuf, const int len)
{
    /*
    if (len < 7)
    {
...
void CMsgBusystate::OnRemoteSourceUsageReport11(unsigned char* pBuf, const int len)
{
    /*
    if (len < 7)
    {
...
void CMsgBusystate::SendLocalSourceUsageReport11()
{
    /*
    m_vBuffer.clear();
    m_Mid = 0x41;
    ...
    CRouteManager *pData = &g_SDKServer.m_routeManager;
    ...
    for (int i = 0; i < NumOfEl; i++)
    {
        ...
        CAnnouncementMicr* pMic = (CAnnouncementMicr*)p->m_pActivate;

        WriteLONG(0x00);                                    //CS call number
        WriteLONG(g_SDKServer.GetNode());      //CSNode
...
void CMsgBusystate::SendRemoteSourceUsageReport11()
{
    /*
    m_vBuffer.clear();
    m_Mid = 0x41;
    ...
    for (unsigned int i = 0; i < vRemote.size(); i++)
    {
        ...
        CAnnouncement* pAct = p->m_pActivate;
