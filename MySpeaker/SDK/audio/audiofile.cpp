...
bool CAudioFile::Open(std::string strPath)
{
    ...
    {
        LOG_ERROR("audio file type error,path: %s", strPath.c_str());
        return false;
    }
    ...
    {
        LOG_ERROR("audio file not exist,path: %s", strPath.c_str());
        return false;
    }
    ...
    if (!m_hFile)
    {
        LOG_ERROR("audio file open error,path: %s", strPath.c_str());
        return false;
    }
    ...
    if (m_type == AUDIO_ADP)
    {
        ...
        {
            LOG_ERROR("%s", "adp audio file format error");
            return false;
        }
    }
    else if(m_type == AUDIO_WAV)
    {
        ...
        {
            LOG_ERROR("%s", "wav audio file format error");
            return false;
        }
        LOG_DEBUG("Open Wav,Channel-%d Bit-%d Rate-%d",
        ...
        else
        {
            ...
            if(m_st == NULL)
            {
                LOG_ERROR("%s","speex_resampler_init error");
            }
    #endif
        }
    }
